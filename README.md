...
- [x] 폴더 구조 생성
- [x] Edge ONNX 모델 변환
- [x] Sensor Fusion (Kalman Filter)
- [x] MQTT 통신 레이어

### Week 2: AI 모델 통합
- [ ] ViT-Base Fine-tuning (construction safety dataset)
- [ ] LSTM 궤적 예측 모델
- [ ] Autoencoder 이상 탐지
- [ ] Cloud-Edge 통신 통합 테스트

### Week 3: BIM + Archive
- [ ] 3D 포인트 클라우드 생성 (SLAM 시뮬레이션)
- [ ] ICP 정렬 알고리즘
- [ ] 해시 체인 통합
- [ ] 감사 기록 내보내기
...
| 역할 | 담당 모듈 | 일정 |
|-----|---------|------|
| 엣지 개발자 | edge/*.py | W1-W2 |
| 클라우드 개발자 | cloud/*.py, api_server | W2-W3 |
| AI 엔지니어 | LSTM, ViT, Autoencoder | W2-W3 |
| 데이터베이스 | archive/hash_chain.py | W3 |
| QA / 테스트 | tests/e2e_test.py | W4 |
...
### 논문 & 표준
- "Real-time Detection of Construction Safety Hazards" - IEEE Access
- "Edge AI for IoT Systems" - ACM Computing Surveys
...
### 향후 고도화 (버전 2.0+)
- 다중 에이전트 강화학습 (PPO) - 자동 회피 경로 제어
- 진정한 블록체인 구현 - 허이퍼레저 또는 Ethereum
- 엣지에서의 ViT 추론 최적화
- 실시간 모델 재학습

### 알려진 제한사항
- 오프라인 환경에서 YOLOv8 다운로드 불가 (인터넷 필요)
- BIM 데이터 부재 시 시뮬레이션 데이터 사용
- NPU 하드웨어 호환성 검증 필요
- MQTT 브로커 없을 시 HTTP fallback 사용

---

## 💡 문제 해결 (Troubleshooting)

### Q: MQTT 브로커에 연결할 수 없음
**A**: 
- Fallback으로 HTTP 프로토콜 사용 (`communication_protocol="http"`)
- 테스트 용 MQTT 브로커: `mqtt.example.com:1883`

### Q: YOLOv8 모델 다운로드 실패
**A**:
- 오프라인 환경: 사전 변환된 ONNX 모델 사용
- 인터넷 환경: `python edge/yolov8_to_onnx.py`

### Q: API 서버 포트 충돌
**A**:
- 다른 포트로 실행: `uvicorn cloud.api_server:app --port 8001`

### Q: Hash Chain DB 손상
**A**:
- `archive/hash_chain.db` 삭제 후 재생성
- 또는 백업에서 복구
